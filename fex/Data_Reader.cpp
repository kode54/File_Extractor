...
blargg_err_t Data_Reader::skip( uint64_t count )
{
	char buf [512];
	while ( count )
	{
...
uint64_t File_Reader::remain() const { return size() - tell(); }

blargg_err_t File_Reader::skip( uint64_t n )
{
	assert( n >= 0 );
	if ( !n )
		return 0;
...
uint64_t Subset_Reader::remain() const { return remain_; }
...
uint64_t Remaining_Reader::remain() const { return header_end - header + in->remain(); }
...
uint64_t Mem_File_Reader::size() const { return size_; }
...
uint64_t Mem_File_Reader::tell() const { return pos; }

blargg_err_t Mem_File_Reader::seek( uint64_t n )
{
	if ( n > size_ )
		return eof_error;
	pos = n;
...
uint64_t Callback_Reader::remain() const { return remain_; }
...
uint64_t Std_File_Reader::size() const
{
	long pos = tell();
...
uint64_t Std_File_Reader::tell() const { return ftell( (FILE*) file_ ); }

blargg_err_t Std_File_Reader::seek( uint64_t n )
{
