...
public:
	...
	virtual uint64_t remain() const BLARGG_PURE( { return 0; } )

	// Reads and discards n bytes. Skipping past end of file results in eof_error.
	virtual blargg_err_t skip( uint64_t n );
...
public:
	// Size of file
	virtual uint64_t size() const BLARGG_PURE( { return 0; } )

	// Current position in file
	virtual uint64_t tell() const BLARGG_PURE( { return 0; } )

	// Goes to new position
	virtual blargg_err_t seek( uint64_t ) BLARGG_PURE( { return 0; } )
